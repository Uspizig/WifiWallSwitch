...
#define WEIGHTING         None // Also avaliable: 'C_weighting' or 'None' (Z_weighting)
...
// 
  ...
  //const GFXfont* Schrift5 = &Aclonica_Regular_16;
  ...
  //GxEPD2_BW<GxEPD2_290, GxEPD2_290::HEIGHT> gfx(GxEPD2_290(/*CS=5*/ E_INK_PIN_SPI_CS, /*DC=*/ E_INK_PIN_SPI_DC, /*RST=*/ E_INK_PIN_SPI_RST, /*BUSY=*/ E_INK_PIN_SPI_BUSY));
  GxEPD2_BW<GxEPD2_213_B74, GxEPD2_213_B74::HEIGHT> gfx(GxEPD2_213_B74(/*CS=5*/ E_INK_PIN_SPI_CS, /*DC=*/ E_INK_PIN_SPI_DC, /*RST=*/ E_INK_PIN_SPI_RST, /*BUSY=*/ E_INK_PIN_SPI_BUSY));
#endif
...
int display_period = 60000;
int meas_period = 1000;
unsigned long time_now = 0;
unsigned long time_now2 = 0;
double short_Leq_dB = 0;
sum_queue_t q;
...
void setup() {
 ...
 /*while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    short_Leq_dB = MIC_OFFSET_DB + MIC_REF_DB + 20 * log10(sqrt(double(q.sum_sqr_weighted) / SAMPLES_SHORT) / MIC_REF_AMPL);
      ...
      if (Leq_dB > MIC_OVERLOAD_DB) {// Display 'Overload' if dB value is over the AOP
        //gfx.setCursor(10, 200); gfx.print("Overload");
      } else if (isnan(Leq_dB) || (Leq_dB < MIC_NOISE_DB)) {// Display 'Low' if dB value is below noise floor
        //gfx.setCursor(10, 200); gfx.print("Low");
      }
         
      // The Leq numeric decibels
      gfx.setCursor(10, 200); gfx.print(String(Leq_dB, 1));  gfx.print(" "); gfx.print(DB_UNITS);
      gfx.setCursor(10, 220); gfx.print(millis()/1000); gfx.print(" Sek");
...
void loop() {
    #if (USE_DISPLAY > 0)
    if(millis() >= time_now + display_period){
        time_now += display_period;
    ...
    #endif  
    if(millis() >= time_now2 + meas_period){
        time_now2 += meas_period;
        #if (USE_DISPLAY > 0)
          rechne_wert();
        #endif  
    }
...
void rechne_wert(void){
  xQueueReceive(samples_queue, &q, portMAX_DELAY);
  // Calculate dB values relative to MIC_REF_AMPL and adjust for microphone reference
    double short_RMS = sqrt(double(q.sum_sqr_SPL) / SAMPLES_SHORT);
    double short_SPL_dB = MIC_OFFSET_DB + MIC_REF_DB + 20 * log10(short_RMS / MIC_REF_AMPL);
  ...
  void zeige_wert_display(void){
        ...
        // The Leq numeric decibels
        gfx.fillRect(10, 185, 90, 210, GxEPD_WHITE);
        gfx.setCursor(10, 200); gfx.print(String(Leq_dB, 1));  gfx.print(" "); gfx.print(DB_UNITS);
        gfx.setCursor(10, 220); gfx.print(millis()/1000); gfx.print(" Sek");
        //display.drawString(0, 4, String(Leq_dB, 1) + " " + DB_UNITS);
